...
def evaluate_model():
    ...
    if val_dir.exists() and len(list(val_dir.iterdir())) > 0:
        ...
        if class_names:
            target_names = [class_names.get(i, f"Class_{i}") for i in range(len(class_names))]
        ...
        dummy_labels = np.eye(len(DISEASE_CLASSES))[:32] if len(DISEASE_CLASSES) >= 32 else np.eye(len(DISEASE_CLASSES))[:len(DISEASE_CLASSES)]
