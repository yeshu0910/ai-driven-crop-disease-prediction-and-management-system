class DiseaseKnowledgeBase:
    ...
    def search(self, query):
        ...
        for disease_name, info in self.database.items():
            if (
                query in disease_name.lower()
                or any(query in s.lower() for s in info.get("symptoms", []))
                or any(query in c.lower() for c in info.get("causes", []))
    ...
    def filter_by_crop(self, crop_name):
        ...
        results = [
            ...
            if crop_name in name.lower()
            or any(crop_name in c.lower() for c in info.get("affected_crops", []))
        ]
        return results if results else list(self.database.keys())
