...
class WeatherAPI:
    ...
    def _is_city_name(self, location):
        if not location:
            return True
        return "," not in location or not any(
            c.isdigit() for c in location.split(",")[0]
        )
            ...
            forecasts.append(
                {
                    ...
                    "description": max(
                        set(dd["descriptions"]), key=dd["descriptions"].count
                    ),
    ...
    def get_disease_risk_from_weather(self, weather_data, crop_name):
        ...
        if 20 <= temperature <= 30:
            risk_score += 20
            risk_factors.append(
                f"Optimal temperature ({temperature}°C) for many pathogens"
            )
        elif temperature > 35:
            risk_score += 10
