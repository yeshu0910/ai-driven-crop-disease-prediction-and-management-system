...
class RecommendationEngine:
    ...
    def generate_recommendations(self, crop_name, disease_name, severity,
                                 infection_percentage, weather_data=None):
        ...
        fertilizers = self._get_fertilizer_suggestions(crop_name, disease_name, severity)
        irrigation = self._get_irrigation_guidance(crop_name, disease_name, severity, weather_data)
        prevention = self._get_prevention_measures(disease_info, severity, weather_data)
        management = self._get_crop_management_tips(crop_name, disease_name, severity, weather_data)
    ...
    def _get_organic_treatment(self, disease_info, severity, crop_name):
        return disease_info.get("organic_treatment", [
    ...
    def _get_base_irrigation(self, crop_name, weather_data):
        ...
        base = irrigation_guides.get(crop_name, f"Provide regular irrigation suitable for {crop_name}.")
        return [base]
    ...
    def _get_prevention_measures(self, disease_info, severity, weather_data):
        ...
        if weather_data:
            if weather_data.get("humidity", 50) > 75:
                extra.append("High humidity period - increase preventive fungicide frequency")
            if weather_data.get("rainfall", 0) > 5:
                extra.append("Rain expected - apply protective treatment before rain")
        return base + extra

    def _get_crop_management_tips(self, crop_name, disease_name, severity, weather_data):
        tips = [
