...
class PDFGenerator:
    ...
    def _build_header(self, data):
        ...
        elements.append(
            Paragraph(
                "AI-DRIVEN CROP DISEASE DIAGNOSIS REPORT", self.styles["CustomTitle"]
            )
        )
        ...
        elements.append(
            Paragraph(
                f"Report ID: {data.get('report_id', 'N/A')}", self.styles["SmallText"]
            )
        )
        ...
        header_data = [
            ...
            [
                "Scan Date",
                data.get("scan_date", datetime.now().strftime("%Y-%m-%d %H:%M")),
            ],
        ]
    ...
    def _build_image_section(self, image_data):
        ...
        if hasattr(image_data, "save"):
            ...
            elements.append(
                Paragraph("Image data not available", self.styles["InfoText"])
            )
    ...
    def _build_diagnosis_section(self, data):
        elements = []
        elements.append(
            Paragraph("Disease Diagnosis Results", self.styles["CustomHeading"])
        )
    ...
    def _build_weather_section(self, weather_data):
        ...
        elements.append(
            Paragraph(
                "Weather Conditions at Time of Scan", self.styles["CustomHeading"]
            )
        )
    ...
    def _build_treatment_section(self, data):
        elements = []
        elements.append(
            Paragraph("Treatment Recommendations", self.styles["CustomHeading"])
        )
        ...
        if treatment:
            for category, recs in treatment.items():
                if recs and isinstance(recs, list):
                    ...
                    elements.append(Paragraph(category_name, self.styles["SubHeading"]))
                    for _i, rec in enumerate(recs, 1):
                        elements.append(
                            Paragraph(f"&#8226; {rec}", self.styles["InfoText"])
                        )
                    elements.append(Spacer(1, 3 * mm))
        else:
            elements.append(
    ...
    def _build_explanation_section(self, explanation):
        elements = []
        elements.append(
            Paragraph("AI Diagnostic Explanation", self.styles["CustomHeading"])
        )

        rationale = explanation.get("prediction_rationale", [])
        if rationale:
            elements.append(
                Paragraph("Why this diagnosis was made:", self.styles["SubHeading"])
            )
            for reason in rationale:
                elements.append(Paragraph(f"&#8226; {reason}", self.styles["InfoText"]))

        confidence_analysis = explanation.get("confidence_analysis", {})
        if confidence_analysis:
            elements.append(Spacer(1, 3 * mm))
            elements.append(
                Paragraph("Confidence Analysis:", self.styles["SubHeading"])
            )
            confidence_items = [
            ...
            for item in confidence_items:
                elements.append(Paragraph(f"&#8226; {item}", self.styles["InfoText"]))
