...
def render_disease_card(disease_name, info):
    ...
    <div class="card" style="margin-bottom:1rem;padding:1.5rem;">
        ...
        <p><strong>Affected Crops:</strong> {affected_text}</p>
        <p><strong>Favorable Conditions:</strong> {favorable}</p>
    </div>
    """, unsafe_allow_html=True)
...
def main():
    ...
    st.markdown(
        f"<p style='color:#888;margin-bottom:1rem;'>Showing {len(diseases)} records</p>",
        unsafe_allow_html=True
    )
