...
def render_current_weather(weather_data):
    ...
    metrics = [
        ...
        ("☁️", "Condition", str(weather_data.get('weather_description', 'N/A')).title(), "#fb8c00"),
...
def render_forecast(forecast_data):
    st.markdown("<h3 class='section-title gradient-text'>📅 7-Day Weather Forecast</h3>", unsafe_allow_html=True)
    ...
    fig2.add_trace(go.Bar(
        ...
        name="Humidity", marker_color="#1e88e5",
        opacity=0.7, text=df["humidity_avg"].round(1),
...
def render_disease_risk(risk_prediction, crop_name):
    st.markdown("<h3 class='section-title gradient-text'>⚠️ Disease Risk Forecast</h3>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"""
            <div class="card" style="text-align:center;height:100%;">
                <div style="font-size:2.5rem;color:{risk_color};font-weight:800;">{risk_level}</div>
                <div class="stat-label" style="font-size:0.8rem;">Overall Risk Level</div>
                <div style="font-size:1.3rem;font-weight:700;color:{risk_color};">{risk_score:.1f}%</div>
...
def render_weather_history(db):
    st.markdown("<h3 class='section-title gradient-text'>📊 Weather History Log</h3>", unsafe_allow_html=True)
...
def main():
    ...
    if not weather_api.is_configured():
        st.warning("⚠️ OpenWeather API key not configured. Weather data will use simulated values. Add `OPENWEATHER_API_KEY=your_key` to `.env` file for real weather data.")
    ...
    if st.button("🌤️ Update Weather & Risk Data", type="primary", width='stretch'):
        if not location:
            st.error("Please enter a location to fetch weather data.")
        else:
            with st.spinner("Fetching weather data..."):
                try:
                    ...
                    if weather_data and weather_data.get("source") == "api":
                        st.success(f"✅ Real weather data loaded for {location}!")
                    else:
                        st.info(f"ℹ️ Using simulated weather data for {location}. Add an API key for live data.")
